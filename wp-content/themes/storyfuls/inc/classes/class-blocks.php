...
class Blocks {
	...
	protected function setup_hooks() {
		...
		add_filter( 'block_categories_all', array( $this, 'storyful_custom_block_category' ) );
	...
	public function register_theme_blocks() {
		...
		if ( file_exists( STORYFUL_SRC_BLOCK_DIR_PATH ) ) {
			$block_json_files = glob( STORYFUL_SRC_BLOCK_DIR_PATH . '/*/block.json' );
	 ...
	 * @since 1.0.0
	 */
	public function storyful_custom_block_category( $categories ) {
		return array_merge(
			array(
				array(
					'slug'  => 'storyful',
					'title' => __( 'storyful', 'storyful' ),
	...
	public static function block_class_from_string( string $string ): ?string {
		...
		if ( false !== strpos( $string, 'storyful/' ) ) {
			$string = str_replace( 'storyful/', '', $string );
		}
	...
	public function localize_gutenberg_blocks_config() : void {
		$blocks_config = apply_filters( 'storyful_gutenberg_blocks_config', array() );

		wp_localize_script(
			'wp-edit-post',
			'storyful_gutenberg_blocks_config',
			$blocks_config
		);
	}
}
