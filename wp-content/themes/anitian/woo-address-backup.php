        ...
        if ( $external_shipping_checked ) {
            for ( $i = 1; $i <= $quantity; $i++ ) {
                ...
                if ( isset( $_POST[ 'external_shipping_checkbox_' . $cart_item_key . '_' . $i ] ) && $_POST[ 'external_shipping_checkbox_' . $cart_item_key . '_' . $i ] == '1' ) {
                    ...
                    if ( isset( $_POST[ 'external_shipping_state_' . $cart_item_key . '_' . $i . '_field' ] ) && $_POST[ 'external_shipping_state_' . $cart_item_key . '_' . $i . '_field' ] == 'visible' ) {
                        // Check if the country has states
                        $country_code = $_POST[ 'external_shipping_country_' . $cart_item_key . '_' . $i ];
                        $has_states = WC()->countries->states_required_for_country( $country_code );

                        // Validate the state field only if the country has states
