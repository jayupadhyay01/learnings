...
class Assets {
	...
	public function move_scripts_to_footer() {
		...
		wp_enqueue_script( 'jquery', '/wp-includes/js/jquery/jquery.min.js', array(), ANITIAN_THEME_VERSION, true );
	}
	...
	public function hook_critical_css() {
		...
		$response = wp_remote_get( ANITIAN_BUILD_URI . '/inline.css' );   // load template output in buffer.

		if ( ! is_wp_error( $response ) ) {
			$css = wp_remote_retrieve_body( $response );
			wp_register_style( 'anitian-inline-css', false, array(), true, true );
			wp_add_inline_style( 'anitian-inline-css', $css );
			wp_enqueue_style( 'anitian-inline-css' );
		}
	}
	...
	public function register_styles() {
		...
		// Register styles.
		wp_register_style( 'main-css', ANITIAN_BUILD_URI . '/main.css', array(), filemtime( ANITIAN_BUILD_PATH . '/main.css' ), 'all' );
	...
	public function register_scripts() {
		// Register scripts.
		wp_register_script( 'main-js', ANITIAN_BUILD_URI . '/main.js', array( 'jquery' ), filemtime( ANITIAN_BUILD_PATH . '/main.js' ), true );
	...
	protected function is_slick_block() { 
		if ( has_block( 'anitian/anitian-resources' ) || has_block( 'anitian/timeline' ) || has_block( 'anitian/real-facts' ) || has_block( 'anitian/customer-stories' ) || has_block( 'anitian/aws-partners' )) {
			return true;
		} else {
			return false;
		}
	...
	public function enqueue_editor_assets() {
		...
		if ( is_admin() ) {
			wp_enqueue_style(
				'anitian-editor-css',
				ANITIAN_BUILD_URI . '/editor.css',
				array(),
				filemtime( ANITIAN_BUILD_PATH . '/editor.css' ),
				'all'
			);
		}
		...
		if ( is_admin() || $this->is_slick_block() ) {
			...
			wp_enqueue_script(
				'anitian-slick',
				ANITIAN_BUILD_LIBRARY_URI . '/js/slick.min.js',
				array( 'jquery' ),
				filemtime( ANITIAN_BUILD_LIBRARY_DIR_PATH . '/js/slick.min.js' ),
				true
			);
		}
		if ( is_admin() || $this->is_slick_block() ) {
			wp_enqueue_style(
				'anitian-slick',
				ANITIAN_BUILD_LIBRARY_URI . '/css/slick.min.css',
				array(),
				filemtime( ANITIAN_BUILD_LIBRARY_DIR_PATH . '/css/slick.min.css' ),
				'all'
			);
		}
