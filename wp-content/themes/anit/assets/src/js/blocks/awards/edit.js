...
export default function Edit( {
		...
		wp.apiFetch( { path: 'anitian_api/request/awards' } ).then(
			( items ) => {
				items.forEach( function ( item ) {
					postList.push( {
						label: __( item.title, 'anitian' ),
						value: __( item.id, 'anitian' ),
					} );
				} );
				setPosts( postList );
			}
	...
	const initslider = () => {
		if (
			! jQuery( '.slider-for-awards' ).hasClass( 'slick-initialized' )
		) {
			setTimeout( () => {
				jQuery( '.slider-for-awards' ).slick( {
					autoplay,
	...
	const reloadSlider = () => {
		const slider = jQuery( '.slider-for-awards' );
