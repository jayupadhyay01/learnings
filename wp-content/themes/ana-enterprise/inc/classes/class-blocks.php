...
class Blocks {
	...
	protected function setup_hooks() {
		...
		add_filter( 'block_categories_all', array( $this, 'ana_enterprise_custom_block_category' ) );
	...
	public function register_theme_blocks() {
		...
		if ( file_exists( ANA_ENTERPRISE_SRC_BLOCK_DIR_PATH ) ) {
			$block_json_files = glob( ANA_ENTERPRISE_SRC_BLOCK_DIR_PATH . '/*/block.json' );
	 ...
	 * @since 1.0.0
	 */
	public function ana_enterprise_custom_block_category( $categories ) {
		return array_merge(
			array(
				array(
					'slug'  => 'ana-enterprise',
					'title' => __( 'ANA Enterprise', 'ana-enterprise' ),
	...
	public static function block_class_from_string( string $string ): ?string {
		...
		if ( false !== strpos( $string, 'ana-enterprise/' ) ) {
			$string = str_replace( 'ana-enterprise/', '', $string );
		}
	...
	public function localize_gutenberg_blocks_config() : void {
		$blocks_config = apply_filters( 'ana_enterprise_gutenberg_blocks_config', array() );

		wp_localize_script(
			'wp-edit-post',
			'ana_enterprise_gutenberg_blocks_config',
			$blocks_config
		);
	}
}
